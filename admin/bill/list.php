    ...
    <div class="row frmcontent">
        <table border="1" style="width:100%; border-collapse: collapse; text-align: center; font-family: sans-serif;">
            ...
            foreach ($listbill as $bill) {
                ...
                // Đảm bảo lấy đúng cột bill_status từ database
                $status_val = $bill_status;

                // Thiết lập màu sắc hiển thị
                $colors = [0 => "#007bff", 1 => "#ffc107", 2 => "#17a2b8", 3 => "#28a745", 4 => "#dc3545"];
                $labels = [0 => "Mới", 1 => "Xử lý", 2 => "Đang giao", 3 => "Hoàn tất", 4 => "Hủy đơn"];
                $current_color = $colors[$status_val] ?? "#000";
                $current_text = $labels[$status_val] ?? "Không rõ";

                $xoabill = "index.php?act=xoabill&idhd=" . $idhd;
                $ctbill = "index.php?act=billdetail&idhd=" . $idhd;

                echo '<tr>
            <td><b>HB-' . $idhd . '</b></td>
            <td>' . $hoten . '</td>
            <td>' . $sdt . '</td>
            <td><b>' . number_format($tongthanhtoan) . ' đ</b></td>
            <td>' . $ngaydat . '</td>
            <td><span style="color:' . $current_color . '; font-weight:bold;">' . $current_text . '</span></td>
            <td>
                <form action="index.php?act=update_status" method="post">
                    ...
                    <select name="new_status" style="padding: 2px;">
                        <option value="0" ' . ($status_val == 0 ? "selected" : "") . '>Mới</option>
                        <option value="1" ' . ($status_val == 1 ? "selected" : "") . '>Xử lý</option>
                        <option value="2" ' . ($status_val == 2 ? "selected" : "") . '>Đang giao</option>
                        <option value="3" ' . ($status_val == 3 ? "selected" : "") . '>Hoàn tất</option>
                        <option value="4" ' . ($status_val == 4 ? "selected" : "") . '>Hủy đơn</option>
                    </select>
                    <input type="submit" name="capnhat_tt" value="Lưu" style="cursor:pointer;">
                </form>
            </td>
            <td>
                <a href="' . $ctbill . '"><input type="button" value="Chi tiết"></a>
                <a href="' . $xoabill . '" onclick="return confirm(\'Xóa đơn hàng này?\')"><input type="button" value="Xóa"></a>
            </td>
        </tr>';
            }
            ?>
